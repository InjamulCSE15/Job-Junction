...
export const FeaturedJobCard = ({ featuredJob }) => {
    const { id, job_title, company, logo, jobType, salary, position, remote, job_location, job_short_details } = featuredJob;
    const location = useLocation();

    // const constructLinkPath = (id) => {
    //     if (location.pathname === '/' || location.pathname.startsWith('/jobs') || location.pathname.startsWith('/applied')) {
    //         return `/job/${id}`;
    //     }
    //     return `/job/${id}`; // Default case
    // };
    // console.log(constructLinkPath);
    return (
        <div className="m-1">
                        ...
                        <div className="flex items-end justify-between">
                            ...
                            <div className="text-end">
                                <h4 className="flex items-center justify-end">
                                    ...
                                    <span className=" text-gray-600 text-sm"> {job_location}</span></h4>
                                {/* <Link to={constructLinkPath(id)} className="btn-link no-underline text-sm bg-gray-800 px-2 py-1 text-white rounded-md hover:border hover:border-gray-800 hover:bg-white hover:text-gray-800 hover:no-underline hover:font-medium">View Details</Link> */}
